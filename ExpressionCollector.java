...
public class ExpressionCollector { 
	 ...
	 */
	protected ArrayList<Expression> exprList = new ArrayList<Expression>();
	...
	boolean isTypeCorrect() {
		...
		for(int i = 0; i < this.exprList.size(); i++) {
			...
			if(this.exprList.get(i).getValue() instanceof Boolean) {
				return true;
			}
		}
		return false;
	...
	Expression[] getExpressions() {
		/* Your Task */
		Expression[] storedExpr = new Expression[this.exprList.size()];
		for(int i = 0; i < this.exprList.size(); i++) {
			storedExpr[i] = this.exprList.get(i);
		}
		return storedExpr;
	}
